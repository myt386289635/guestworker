...
import com.guestworker.bean.CityBean;
...
//        defX.add("围场满族蒙古族自治县");
     ...
     * 根据省份和城市编号获取县 或者区的编码
     */
    public String getCode(int pIndex, int cIndex, int xIndex){
        List<CityBean> city_list = mList.get(pIndex).getCities();
        if (city_list.get(cIndex).getCities().size() != 0){
            //有第三级
            return city_list.get(cIndex).getCities().get(xIndex).getCode();
        }else {
            //没有第三级
            return city_list.get(cIndex).getCode();
        }
    }
